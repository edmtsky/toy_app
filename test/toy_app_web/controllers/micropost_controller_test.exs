defmodule ToyAppWeb.MicropostControllerTest do
  ...
  describe "index" do
    test "lists all microposts", %{conn: conn} do
      conn = get(conn, Routes.micropost_path(conn, :index))
  ...
  describe "new micropost" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.micropost_path(conn, :new))
  ...
  describe "create micropost" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.micropost_path(conn, :create), micropost: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.micropost_path(conn, :show, id)

      conn = get(conn, Routes.micropost_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Micropost"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.micropost_path(conn, :create), micropost: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Micropost"
    end
  end
  ...
  describe "edit micropost" do
    ...
    test "renders form for editing chosen micropost", %{conn: conn, micropost: micropost} do
      conn = get(conn, Routes.micropost_path(conn, :edit, micropost))
  ...
  describe "update micropost" do
    ...
    test "redirects when data is valid", %{conn: conn, micropost: micropost} do
      conn = put(conn, Routes.micropost_path(conn, :update, micropost), micropost: @update_attrs)
      assert redirected_to(conn) == Routes.micropost_path(conn, :show, micropost)

      conn = get(conn, Routes.micropost_path(conn, :show, micropost))
      assert html_response(conn, 200) =~ "some updated content"
    end

    test "renders errors when data is invalid", %{conn: conn, micropost: micropost} do
      conn = put(conn, Routes.micropost_path(conn, :update, micropost), micropost: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Micropost"
    end
  end
  ...
  describe "delete micropost" do
    ...
    test "deletes chosen micropost", %{conn: conn, micropost: micropost} do
      ...
      assert redirected_to(conn) == Routes.micropost_path(conn, :index)

      assert_error_sent 404, fn ->
        get(conn, Routes.micropost_path(conn, :show, micropost))
      end
    end
  end
